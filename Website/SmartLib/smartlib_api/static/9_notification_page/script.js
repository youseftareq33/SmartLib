...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Fetch initial data when page loads
    fetch(`/getNotificationListView?user_id=${userId}`)
        .then(response => {
            if (!response.ok) {
                throw new Error('Failed to fetch Notification');
            }
...
function updateCommentDetails(data) {
    ...
    data.results.forEach((mission, index) => {
        const missionTitle = document.getElementById(`mission-title_${index + 1}`);
        const missionContent = document.getElementById(`mission-content_${index + 1}`);
        const missionImg = document.getElementById(`mission-img_${index + 1}`);
        ...
        if (mission) {
            ...
            missionTitle.textContent = mission.notification_title || 'Title..';
            missionContent.textContent = mission.notification_record || 'Description..';

            // image
            if (mission.notification_title=="Accept Uploaded Book") {
                missionImg.src = '/static/images/accept.png'; 
            }
            else if(mission.notification_title=="Rejected Uploaded Book"){
                missionImg.src = '/static/images/reject.png'; 
            }
            else if(mission.notification_title=="New Point Achievement"){
                missionImg.src = '/static/images/point.png'; 
            }
